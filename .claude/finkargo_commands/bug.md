...
- Create the plan in the `specs/` directory with filename: `issue-{issue_number}-adw-{adw_id}-sdlc_planner-{descriptive-name}.md`
  - Replace `{descriptive-name}` with a short, descriptive name based on the bug (e.g., "fix-contract-download", "resolve-auth-error", "patch-excel-parser")
...
When investigating bugs, understand the layer where the issue occurs:
- **Backend Clean Architecture**: adapter/rest → core/servicios → repositorio
- **Frontend Flow**: pages → components → services → api
- **Components use FK prefix** (e.g., `FKContractRequest.tsx`)
...
- `backend/src/adapter/rest/` - API routes
- `backend/src/core/servicios/` - Business logic
- `backend/src/repositorio/` - Data access
...
<If the bug affects UI, include a task to create a E2E test file. Your task should look like: "Read `.claude/commands/e2e/test_login.md` and `.claude/commands/e2e/test_contract_request.md` and create a new E2E test file in `.claude/commands/e2e/test_<descriptive_name>.md` that validates the bug is fixed, be specific with the steps to prove the bug is fixed. We want the minimal set of steps to validate the bug is fixed and screen shots to prove it if possible.">
...
- Include the full path to the plan file you created (e.g., `specs/issue-123-adw-abc123-sdlc_planner-fix-contract-download.md`)
