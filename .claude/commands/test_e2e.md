...
- IMPORTANT: Ensure the backend and frontend servers are running before executing tests
  - Backend: http://localhost:8000 (FastAPI)
  - Frontend: http://localhost:5173 (Vite)
- Read `scripts/start-dev.sh` and `scripts/stop-dev.sh` for server management
- For tests requiring authentication, use test credentials or Supabase test accounts

## Authentication Handling

Many Finkargo workflows require authentication. For E2E tests:
1. Navigate to login page first if test requires auth
2. Use test account credentials appropriate for the role being tested:
   - Legal tests: use legal role test account
   - Operations tests: use operations role test account
   - Admin tests: use admin role test account
3. Wait for redirect to dashboard after login
4. Verify role-appropriate dashboard appears
